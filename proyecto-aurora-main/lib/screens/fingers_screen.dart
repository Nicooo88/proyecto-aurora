...
class _FingersScreenState extends State<FingersScreen> {
  ...
  void _toggleDedo(String nombre) {
    setState(() {
      _dedos[nombre] = !(_dedos[nombre] ?? false);
      _accionActiva = 'custom';
    });
    widget.onAccion('custom');
  }
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              _buildDedoTappable('pulgar', 38, 55, esPuno),
              const SizedBox(width: 8),
              _buildDedoTappable('indice', 44, 90, esPuno),
              const SizedBox(width: 8),
              _buildDedoTappable('medio', 44, 100, esPuno),
              const SizedBox(width: 8),
              _buildDedoTappable('anular', 44, 85, esPuno),
              const SizedBox(width: 8),
              _buildDedoTappable('menique', 38, 65, esPuno),
            ],
          ),
          Container(
            width: 230,
  ...
  Widget _buildDedoTappable(String nombre, double ancho, double altoMax, bool esPuno) {
    return GestureDetector(
      onTap: esPuno ? null : () => _toggleDedo(nombre),
      child: _buildDedo(nombre, ancho, altoMax, esPuno),
    );
  }
